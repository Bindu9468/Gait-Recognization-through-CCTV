...
def read(csv_path, dataset, metric):
    ...
    df1 = pd.concat(chunk_list)  # DataFrame
...
def get_p_value(arrA, arrB):
    ...
    print(stats.levene(arrA, arrB))  # p>0.05
    t, p = stats.ttest_ind(a, b, equal_var=False)  # equal_var： True()， 2 。 False， Welch t-test
    print(f"T : {t=}, {p=}")
    return p
...
def data_fit(data, path):
    ...
    f = Fitter(data, distributions=distributions, timeout=10000)  # Fitter
    f.fit()  # fit
    ...
    print(f"f.df_errors:")
    print(f"{f.df_errors}")
    print(f"{f.fitted_param['logistic']=}")  # "logistic
    print(f"{f.fitted_param['t']=}")  # "t"
    print(f"{f.fitted_param['johnsonsb']=}") 
    
    f.hist()  # normed histogram
